...
public class Tool {
    ...
    //随机抽取3个人
    public JSONArray RandomThree(HashSet<String> origin_members){
        //刚好3个人就不需要再抽取了
        if (origin_members.size() == 3){
            JSONArray result = new JSONArray();
            for (String username: origin_members
            ) {
                JSONObject reviewer = new JSONObject();
                reviewer.put("username", username);
                reviewer.put("view", false);
                result.add(reviewer);
            }
            return result;
        }
        ...
        return PackageReviewers(reviewers);
    }

    //基于负担平均随机分配
    public String RandomThreeBasedOnBurden(List<Paper> papers, HashSet<String> origin_members) {
        List<String> tmp = new LinkedList<>(origin_members);
        for (Paper paper : papers
             ) {
            HashSet<String> reviewers = new HashSet<>();
            while (reviewers.size() < 3){
                if (tmp.size() == 0){
                    tmp.addAll(origin_members);
                }
                String reviewer = tmp.get(this.rand.nextInt(tmp.size()));
                //之前未分配过这个pcmember给这个paper
                if (!reviewers.contains(reviewer)){
                    reviewers.add(reviewer);
                    tmp.remove(reviewer);
                }
            }
            JSONArray result = PackageReviewers(reviewers);
            paper.setStatus("waiting");
            paper.setReviewers(result.toJSONString());
        }
        return "success";
    }

    public JSONArray PackageReviewers(HashSet<String> reviewers) {
        JSONArray result = new JSONArray();
        List<String> usernames = new ArrayList<>(reviewers);
